...
parser.add_argument("-n", "--ngrid", help="Number of grid cells of sim")

parser.add_argument("--dt", help="Number of seconds (in omega * t) between data output saves")

parser.add_argument(
    "-m", "--multi", help="Generate multi-panel plot of Mach number", action="store_true"
)
...
for f in tqdm(sorted(glob.glob(f"{args.directory}/*.h5")), desc="Getting max and min vals :"):
...
for f in tqdm(
    sorted(glob.glob(f"{args.directory}/*.h5")), desc=f"Plotting {field_key[args.field]} :"
):
    ...
    plt.title(
        rf"{field_key[field]}, $\omega t$ = {round(float(strnum) * float(args.dt),3)}", fontsize=16
    )
...
if args.multi:
    ...
    f_08 = rf"C:\Users\micha\OneDrive\Documents\CTRFL\MAE557\Project2\data\Ma_08\test.output.{strnum}.h5"
    f_04 = rf"C:\Users\micha\OneDrive\Documents\CTRFL\MAE557\Project2\data\Ma_04\test.output.{strnum}.h5"
    f_02 = rf"C:\Users\micha\OneDrive\Documents\CTRFL\MAE557\Project2\data\Ma_02\test.output.{strnum}.h5"
    f_01 = rf"C:\Users\micha\OneDrive\Documents\CTRFL\MAE557\Project2\data\Ma_01\test.output.{strnum}.h5"
    ...
    im_08 = ax[0, 0].pcolormesh(
        X, Y, plotarray_08, cmap="jet", vmax=np.amax(plotarray_08), vmin=np.amin(plotarray_08)
    )
    ax[0, 0].set_title("Ma = 0.8")

    im_04 = ax[0, 1].pcolormesh(
        X, Y, plotarray_04, cmap="jet", vmax=np.amax(plotarray_04), vmin=np.amin(plotarray_04)
    )
    ax[0, 1].set_title("Ma = 0.4")

    im_02 = ax[1, 0].pcolormesh(
        X, Y, plotarray_02, cmap="jet", vmax=np.amax(plotarray_02), vmin=np.amin(plotarray_02)
    )
    ax[1, 0].set_title("Ma = 0.2")

    im_01 = ax[1, 1].pcolormesh(
        X, Y, plotarray_01, cmap="jet", vmax=np.amax(plotarray_01), vmin=np.amin(plotarray_01)
    )
    ax[1, 1].set_title("Ma = 0.1")
